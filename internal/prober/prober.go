...
func SpeedTest(ctx context.Context, r *result.Result, cfg Config, bytes int64) error {
	...
	r.SpeedTested = true
	r.SpeedTestError = ""
	throughput, downloaded, elapsed, err := probeDownload(ctx, r.IP, r.Port, cfg.Timeout, bytes, cfg.InsecureSkipVerify)
...
func probeHTTP(ctx context.Context, ip net.IP, port int, sni string, timeout time.Duration, speedBytes int64, insecure bool, wsHost, wsPath string, requireWS bool) (
	...
	if httpStatus >= 200 && httpStatus < 400 && colo != "" {
		if speedBytes > 0 {
			throughput, _, _, _ = probeDownload(ctx, ip, port, timeout, speedBytes, insecure)
		}
...
func probeDownload(ctx context.Context, ip net.IP, port int, timeout time.Duration, bytes int64, insecure bool) (float64, int64, time.Duration, error) {
	if bytes <= 0 {
