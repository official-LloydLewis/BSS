...
func TestSpeedTestUsesDedicatedTimeoutFloor(t *testing.T) {
	ips := make(chan net.IP, 1)
	ips <- net.ParseIP("192.0.2.1")
	close(ips)
	var gotTimeout time.Duration
	runConfigPortProbesWithProbe(context.Background(), ips, []int{443}, 1, prober.Config{Port: 443, Mode: prober.ModeHTTP, Timeout: time.Second}, func(*result.Result) {}, neighborScanOpts{speedBytes: config.SpeedTestBytes, maxSpeedCandidates: 1}, func(_ context.Context, ip net.IP, cfg prober.Config) *result.Result {
		return &result.Result{IP: ip, Port: cfg.Port, ProbeMode: "http", Latencies: []time.Duration{time.Millisecond}, TLSOk: true, HTTPStatus: 200, Colo: "FRA"}
	}, func(_ context.Context, r *result.Result, cfg prober.Config, _ int64) error {
		gotTimeout = cfg.Timeout
		r.SpeedTested = true
		r.Throughput = 1
		return nil
	})
	if gotTimeout != config.SpeedTestTimeout {
		t.Fatalf("speed timeout = %s, want %s", gotTimeout, config.SpeedTestTimeout)
	}
}
